...
@Composable
fun FounderEditScreen(founderId: String?, onDone: () -> Unit, viewModel: FounderEditViewModel = viewModel()) {
    val state by viewModel.uiState.collectAsState()
    val context = LocalContext.current

    LaunchedEffect(founderId) { viewModel.load(founderId) }
    ...
    Scaffold(topBar = { TopAppBar(title = { Text(if (founderId == null) "Add Leader" else "Edit Leader") }) }) { padding ->
        Column(
